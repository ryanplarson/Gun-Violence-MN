...
prepost_cj <- cj_exp_tract_year %>%
  mutate(pre_post = ifelse(year < 2020, "pre", "post")) %>%
  group_by(GEOID, pre_post) %>%
  summarize(car_jack = sum(car_jack, na.rm = T),
            total_pop = sum(total_pop, na.rm = T)) %>%
  mutate(car_jack_rate = car_jack/total_pop*1000) %>%
  select(GEOID, pre_post, car_jack, car_jack_rate) %>% 
  pivot_wider(names_from = pre_post, values_from = c(car_jack, car_jack_rate))

output <- crimedispersion(as.data.frame(prepost_cj), 'GEOID', 'car_jack_pre', 'car_jack_post')
...
```{r}
prepost_hom <- homicide_tract_year %>%
  mutate(pre_post = ifelse(year < 2020, "pre", "post")) %>%
  group_by(GEOID, pre_post) %>%
  summarize(homicide = sum(homicide, na.rm = T),
            total_pop = sum(total_pop, na.rm = T)) %>%
  mutate(homicide_rate = homicide/total_pop*1000) %>%
  select(GEOID, pre_post, homicide, homicide_rate) %>% 
  pivot_wider(names_from = pre_post, values_from = c(homicide, homicide_rate))

output_homicide <- crimedispersion(as.data.frame(prepost_hom), 'GEOID', 'homicide_pre', 'homicide_post')
...
cj_delta <- prepost_cj %>%
  mutate(delta = car_jack_post-car_jack_pre) %>%
  st_sf()

nb <- st_contiguity(cj_delta, queen=TRUE)
wt <-  st_weights(nb, style = "W")
global_moran(cj_delta$delta, nb, wt)
global_moran_test(
  cj_delta$delta,
  nb,
  wt,
...
cj_lisa <-  local_moran(cj_delta$delta,
                       nb = nb,
                       wt = wt) %>%
  mutate(mean_p = ifelse(p_ii <= 0.05, as.character(mean), "Non Sig."))

cj_delta %>% cbind(cj_lisa) %>%
