...
# ACS Covariates and Denominators
...
```{r}
cj_exp <- read_csv("Car Jacking/MPDdata_2017to2022.csv") %>%
   mutate(date=mdy_hm(reporteddate),
         year=isoyear(date),
         week=isoweek(date)) %>%
  select(casenumber, year, week, latitude, longitude) %>%
  distinct(casenumber, .keep_all = TRUE) %>% #collapsing to incident-level
...
ggplot(cj_exp_week)+
  ...
  geom_vline(xintercept=cj_exp_week$begin_date[cj_exp_week$year==2020 & 
   ...
   geom_label(aes(x=cj_exp_week$begin_date[cj_exp_week$year==2020 &
             ...
             fontface = "bold")+
  labs(title = "Weekly Minneapolis Carjackings, 2017-2022",
       x = "Week",
...
ggsave(filename = "Car Jacking/Figures for publication/fig1.png", bg="white", width = 10, height = 8)
```

# Structural Change in Carjacking

```{r}
library(strucchange)

cj_exp_week <- cj_exp_week %>%
  ungroup() %>%
  mutate(t = row_number())

sctest(cj_exp_week$car_jack_rate~cj_exp_week$t, 
       type = "Chow",
       point = 180)

breakpoints(car_jack_rate~t,
            data = cj_exp_week, 
            breaks = 1)
```
...
#george floyd square
gfs_label <- geocode("George Floyd Square, Minneapolis", output = "latlon") %>%
...
ggplot() +
  ...
  geom_text_repel(data = gfs_label, aes(x=lon, y=lat, label = name),
                 size = 2,
                 fontface = "bold",
                 nudge_x = .1, nudge_y = -.06)+
  facet_grid(~year)+
  scale_fill_distiller(palette = "Spectral")+
  labs(title = "Minneapolis Carjacking Rates by Tract and Year", 
       fill = "Weekly Rate/ 1,000")+
  theme_void()+
  theme(legend.position="bottom")


ggsave(filename = "Car Jacking/Figures for publication/fig2.png", bg="white", width = 10, height = 8)
```
...
ggplot(homicide_week)+
 ...
 scale_x_date(date_labels = "%b-%Y", date_breaks = "15 weeks",
     ...
     labs(title = "Weekly Minneapolis Homicide, 2017-2022",
       x = "Week",
  ...
  geom_vline(xintercept=homicide_week$begin_date[homicide_week$year==2020 & 
   ...
   geom_label(aes(x=homicide_week$begin_date[homicide_week$year==2020 &
                 ...
                 y=0.015), 
             label = "George Floyd", show.legend = FALSE)+
  geom_line(aes(x=begin_date, y=csma, color = "CSMA(5)"))+     theme_classic()+
  theme(axis.text = element_text(face = "bold", size = 12),
        axis.title.x = element_text(face = "bold", size = 12),
        axis.title.y = element_text(face = "bold", size = 12),
        legend.key.size = unit(0.8, "cm"),
...
ggsave(filename = "Car Jacking/Figures for publication/fig3.png", bg="white", width = 10, height = 8)
```

# Structural Change in Carjacking

```{r}
homicide_week <- homicide_week %>%
  ungroup() %>%
  mutate(t = row_number())

sctest(homicide_week$homicide_rate~homicide_week$t, 
       type = "Chow",
       point = 180)

breakpoints(homicide_rate~t,
            data = homicide_week, 
            breaks = 1)
```
...
ggplot() +
  ...
  facet_grid(~year)+
  scale_fill_distiller(palette = "Spectral")+
  labs(title = "Minneapolis Homicide Rates by Tract and Year", 
       fill = "Weekly Rate/ 1,000")+
  theme_void()+
  theme(legend.position="bottom")

ggsave(filename = "Car Jacking/Figures for publication/fig4.png", bg="white", width = 10, height = 8)
```
...
ggsave(filename = "Car Jacking/Figures for publication/fig5.png", bg="white", width = 10, height = 8)
```
...
output_homicide[[2]]
ggsave(filename = "Car Jacking/Figures for publication/fig6.png", bg="white", width = 10, height = 8)
...
mp <- moran.plot(cj_delta$delta, nb2listw(nb), 
                 labels = as.character(cj_delta$GEOID))
...
ggplot(mp, aes(x=x, y=wx)) + 
  geom_point(shape=1) + 
    ...
    theme_minimal() + 
    labs(title = "Moran's I Plot Carjacking Change Spatialautocorrelation",
         x = "Focal Tract Carjacking Change",
         y = "Spatially Lagged Carjacking Change")

ggsave(filename = "Car Jacking/Figures for publication/MI_CJ.png", bg="white", width = 10, height = 8)

global_moran_test(
  cj_delta$car_jack_rate_1,
  nb,
  wt,
  alternative = "greater",
  randomization = TRUE)
...
cj_lisa_rate <-  local_moran(cj_delta$car_jack_rate_1,
                       nb = nb,
                       wt = wt, 
                       nsim = 1000,
                       iseed = set.seed(7188)) %>%
...
cj_delta %>% 
  ...
  labs(title = "LISA Plot for Carjacking Change Pre/Post Police Murder",
       fill = "Cluster Type",
       caption = "Clusters significant at p < .05 with 1,000 simulations.")

ggsave(filename = "Car Jacking/Figures for publication/fig7.png", bg="white", width = 10, height = 8)
```
...
hom_delta <- prepost_hom %>%
  mutate(delta = homicide_rate_1-homicide_rate_0,
           GEOID = as.numeric(GEOID)) %>%
  left_join(mpls_tract, by = "GEOID") %>%
  st_sf()
...
mp <- moran.plot(hom_delta$delta, nb2listw(nb))
...
ggplot(mp, aes(x=x, y=wx)) + 
  geom_point(shape=1) + 
    ...
    geom_vline(xintercept=0, lty=2) + 
    theme_minimal() + 
    labs(title = "Moran's I Plot Homicide Change Spatialautocorrelation",
         x = "Focal Tract Homicide Change",
         y = "Spatially Lagged Homicide Change")

ggsave(filename = "Car Jacking/Figures for publication/MI_HOM.png", bg="white", width = 10, height = 8)

global_moran_test(
  hom_delta$homicide_1,
  nb,
  wt,
  alternative = "greater",
  randomization = TRUE)
...
hom_lisa_rate <-  local_moran(hom_delta$homicide_1,
                       nb = nb,
                       wt = wt, 
                       nsim = 1000,
                       iseed = set.seed(7188)) %>%
...
hom_delta %>% 
  ...
  geom_sf(lwd = 0.2, color = "black") +
  theme_void() +
  scale_fill_manual(values = c("red", "yellow", "green","blue", "white"), drop = F)+
  labs(title = "LISA Plot for Homicide Change Pre/Post Police Murder",
       fill = "Cluster Type",
       caption = "Clusters significant at p < .05 with 1,000 simulations.")

ggsave(filename = "Car Jacking/Figures for publication/fig8.png", bg="white", width = 10, height = 8)
```
...
cd_predict <- as.vector(lavPredict(cfa_cd, newdata = as.data.frame(cj_exp_prepost)))
cj_exp_prepost$conc_dis <- cd_predict

cd_model_2  <- ' cd =~ unemp_rate + pov_rate +  female_hh_rate + no_hs_dip_rate 
                  unemp_rate ~~	pov_rate'  

cfa_cd_2 <- cfa(cd_model_2, data = cj_exp_prepost, std.lv = T)
modificationindices(cfa_cd_2)
summary(cfa_cd_2, fit.measures=TRUE, standardized = T)
cd_predict_2 <- as.vector(lavPredict(cfa_cd_2, newdata = as.data.frame(cj_exp_prepost)))
cj_exp_prepost$conc_dis_no_black <- cd_predict_2


write_csv(cj_exp_prepost, file = "cj_exp_prepost.csv")


#predicted probability/rate plots
  #over time
  #stratified by CD (mean, +1SD, -1SD, +2SD, -2SD)


re <- lmer(car_jack_rate~t+post_floyd+t_post_floyd+
             conc_dis+
             age_19_29_perc+age_30_49_perc+age_50_69_perc+
             age_70_plus_perc+ post_floyd:conc_dis+t_post_floyd:conc_dis+
             (1|GEOID), 
          data = cj_exp_prepost)
summary(re)
...
pred_raw <- emmeans(re, c("conc_dis", "post_floyd"), 
                    at = list(conc_dis = c(-2, -1, 0, 1, 2),
                              post_floyd = c(0,1),
                              t_post_floyd = mean(cj_exp_prepost$t_post_floyd[cj_exp_prepost$post_floyd==1]),
                              t = mean(cj_exp_prepost$t[cj_exp_prepost$post_floyd==0]))) %>%
  as.data.frame() %>%
  mutate(conc_dis = factor(case_when(
    conc_dis==-2~"-2 SD CD",
    conc_dis==-1~"-1 SD CD",
    conc_dis==0~"Mean CD",
    conc_dis==1~"+1 SD CD",
    ...
    levels = c("-2 SD CD",
               ...
               "+1 SD CD",
               "+2 SD CD")),
    post_floyd = factor(case_when(
    post_floyd==1~"Post",
    post_floyd==0~"Pre"),
    levels = c("Pre", "Post")))

ggplot(pred_raw, aes(x = post_floyd, y = emmean, color = conc_dis))+
  geom_point(size = 2)+
  geom_errorbar(aes(ymin=asymp.LCL, ymax=asymp.UCL), width = .2, size = 1)+
  labs(title = "Predicted Carjacking Rates by Concentrated Disadvantage, Pre- and Post-Killing",
       y = "Predicted Weekly Carjacking Rate",
       x = "Time Period")+
  facet_grid(~conc_dis)+
  guides(color = "none")+
  theme_classic()+
...
ggsave(filename = "Car Jacking/Figures for publication/fig9.png", bg="white", width = 8, height = 8)
```

### CD Map

```{r, CD map}
cd_cat <- read_csv("Data/cd_noblack_forRyan_031324.csv")


cj_exp_tract_year_cd <- cj_exp_prepost %>%
  filter(year==2020) 
  
q_25 <- quantile(cj_exp_tract_year_cd$conc_dis_no_black, .25)
q_75 <- quantile(cj_exp_tract_year_cd$conc_dis_no_black, .75)


cj_exp_tract_year_cd <- cj_exp_prepost %>%
  mutate(`Concentrated Disadvantage` = case_when(
    conc_dis_no_black < q_25 ~ "Advantaged",
    conc_dis_no_black >= q_25 & conc_dis_no_black <= q_75  ~ "Median",
    conc_dis_no_black > q_75 ~ "Disadvantaged"
  ))

cj_exp_tract_year_cd <-  cj_exp_tract_year_cd %>%
  mutate(GEOID = as.numeric(GEOID)) %>%
  left_join(cd_cat, by = "GEOID")

table(cj_exp_tract_year_cd$`Concentrated Disadvantage.x`==cj_exp_tract_year_cd$`Concentrated Disadvantage.y`)

MyColour <- c("#c1f0e0", "#c8e1ff", "#f5c6d8") 
names(MyColour) <- c("Advantaged", "Median", "Disadvantaged")

ggplot() +
  geom_sf(data = cj_exp_tract_year_cd, 
          aes(geometry = geometry, fill = `Concentrated Disadvantage.x`)) + 
  geom_sf(data = mpls, aes(geometry = geometry), color = "black", alpha = 0)+
  geom_sf(data = gfs, aes(geometry = geometry), color = "black")+
  geom_text_repel(data = gfs_label, aes(x=lon, y=lat, label = name),
                 size = 2,
  ...
  labs(title = "Minneapolis Concentrated Disadvantage by Tract",
       subtitle = "2020 ACS 5-Year Estimates", 
       fill = "Concentrated Disadvantage")+
  theme_void()+
  theme(strip.text.x = element_text(face = "bold", size = 12),
        legend.title = element_text(face = "bold", size = 10))

ggsave(filename = "Car Jacking/Figures for publication/A_CD.png", bg="white", width = 8, height = 8)
```
...
stargazer(results_table, summary = FALSE, header = F,
           ...
           notes="$LR\\chi^2$ vs. saturated (1) = 20.79***, RMSEA = .023 (PCLOSE = 1.0), CFI = 1.0, SRMR = .004")
```

```{r, results='asis'}
results_table<-standardizedSolution(cfa_cd_2) %>% 
...
stargazer(results_table, summary = FALSE, header = F,
           type="latex", style="aer", align = T, 
           title="CFA Measurement Model of Concentrated Disadvantage", 
           notes="$LR\\chi^2$ vs. saturated (2) = 94.97, RMSEA = .035 (PCLOSE = 1.0), CFI = .999, SRMR = .006")
```
  
```{r, message=FALSE}
re_logit_cd <- glmer(anyjack ~ t+post_floyd+t_post_floyd+conc_dis+
             age_19_29_perc+age_30_49_perc+age_50_69_perc+
             age_70_plus_perc+ post_floyd:conc_dis+t_post_floyd:conc_dis+
             (1|GEOID), 
...
pred_prob_raw <- emmeans(re_logit_cd, c("conc_dis", "post_floyd"), 
                    at = list(conc_dis = c(-2, -1, 0, 1, 2),
                              post_floyd = c(0,1),
                              t_post_floyd = mean(cj_exp_prepost$t_post_floyd[cj_exp_prepost$post_floyd==1]),
                              t = mean(cj_exp_prepost$t[cj_exp_prepost$post_floyd==0])),
...
ratios <- pred_prob_raw %>%
  select(conc_dis, post_floyd, prob) %>% 
  pivot_wider(names_from = post_floyd, values_from = prob) %>%
  mutate(ratio = Post/Pre)

pred_prob_raw <- pred_prob_raw %>%
  left_join(ratios, by = "conc_dis") %>%
  mutate(ratio = ifelse(post_floyd=="Pre", NA_integer_, round(ratio,1)),
         ratio = ifelse(is.na(ratio), ratio, paste0(ratio, "x")))

ggplot(pred_prob_raw, aes(x = post_floyd, y = prob, color = conc_dis))+
  geom_point(size = 2)+
  geom_errorbar(aes(ymin=asymp.LCL, ymax=asymp.UCL), width = .2, size = 1)+
  labs(title = "Predicted Carjacking Probabilities by Concentrated Disadvantage, Pre- and Post-Killing",
       y = "Predicted Carjacking Probability",
       x = "Time Period")+
  theme_classic()+
  facet_grid(~conc_dis)+
  guides(color = "none")+
  geom_text(aes(x = post_floyd, y = asymp.UCL+.01, label = ratio),
...
ggsave(filename = "Car Jacking/Figures for publication/fig10.png", bg="white", width = 8, height = 8)
```

```{r, homicide models}
homicide <- homicide %>% st_drop_geometry()
...
re_homicide <- lmer(homicide_rate~t+post_floyd+t_post_floyd+conc_dis+
             age_19_29_perc+age_30_49_perc+age_50_69_perc+
             age_70_plus_perc+ post_floyd:conc_dis+t_post_floyd:conc_dis+
             (1|GEOID), 
          data = cj_exp_prepost)
...
pred_raw_homicide <- emmeans(re_homicide, c("conc_dis", "post_floyd"), 
                    at = list(conc_dis = c(-2, -1, 0, 1, 2),
                              post_floyd = c(0,1))) %>%
  as.data.frame() %>%
  mutate(conc_dis = factor(case_when(
...
ggplot(pred_raw_homicide, aes(x = post_floyd, y = emmean, color = conc_dis))+
  geom_point()+
  geom_errorbar(aes(ymin=asymp.LCL, ymax=asymp.UCL), width=0.2)+
  labs(title = "Predicted Homicide Rates by Concentrated Disadvantage, Pre- and Post-Killing",
       y = "Predicted Weekly Homicide Rate",
       x = "Time Period")+
  theme_classic()+
  facet_grid(~conc_dis)+
  guides(color = "none")

re_logit_cd_homicide <- glmer(anyhom ~ t+post_floyd+t_post_floyd+conc_dis+
             age_19_29_perc+age_30_49_perc+age_50_69_perc+
             age_70_plus_perc+ post_floyd:conc_dis+t_post_floyd:conc_dis+
             (1|GEOID), 
                  data = cj_exp_prepost, family = binomial)
summary(re_logit_cd_homicide)
```
...
stargazer(re, re_logit_cd,
          ...
          covariate.labels = c("T", "Post-Killing", "T Post-Killing",
                              "Conc. Dis.", "Age 19-29", "Age 30-49", 
                              "Age 50-69", "Age 70+", 
                              "Post-Killing X Conc. Dis.",
                              "T Post-Killing X Conc. Dis."),
          header = F,
          ...
          add.lines = list(c("SD(Tract)", .013, .674),
                           c("SD(Residual)",  .094, "")),
          notes.append = F)
```
...
nb <- st_contiguity(mpls_tract, queen=TRUE)
wt <-  st_weights(nb, style = "W", allow_zero = T)
w <- recreate_listw(nb, wt)
...
#write_csv(cj_exp_prepost, "Car Jacking/cj_exp_prepost.csv")

cj_exp_prepost_panel <- pdata.frame(cj_exp_prepost, index = c("GEOID", "WEEKID"), drop.index = F) 

slmtest(car_jack_rate~t+post_floyd+t_post_floyd+
             conc_dis+
             age_19_29_perc+age_30_49_perc+age_50_69_perc+
             age_70_plus_perc+ post_floyd:conc_dis+t_post_floyd:conc_dis,
        data=cj_exp_prepost_panel, 
        listw =w,
        model = "random",
        test = "lml")
...
#SAR Linear CJ
sar <- spml(car_jack_rate~t+post_floyd+t_post_floyd+
             conc_dis+
             age_19_29_perc+age_30_49_perc+age_50_69_perc+
             age_70_plus_perc+ post_floyd:conc_dis+t_post_floyd:conc_dis, 
                      data = cj_exp_prepost, 
...
#SAR LPM CJ
sar_lpm <- spml(anyjack~t+post_floyd+t_post_floyd+
             conc_dis+
             age_19_29_perc+age_30_49_perc+age_50_69_perc+
             age_70_plus_perc+ post_floyd:conc_dis+t_post_floyd:conc_dis, 
                      data = cj_exp_prepost, 
