...
ggplot(mp, aes(x=x, y=wx)) + 
  geom_point(shape=1) + 
    ...
    theme_minimal() + 
    labs(title = "Figure 4: Moran's I Plot Carjacking Change Spatialautocorrelation",
...
cj_lisa <-  local_moran(cj_delta$car_jack_rate_1,
                       nb = nb,
                       wt = wt, 
                       nsim = 1000,
                       iseed = set.seed(7188)) %>%
...
hom_lisa <-  local_moran(hom_delta$homicide_1,
                       nb = nb,
                       wt = wt, 
                       nsim = 1000,
                       iseed = set.seed(7188)) %>%
...
re_lpm <- lmer(anyjack~t+post_floyd+t_post_floyd+
             conc_dis+
             age_19_29_perc+age_30_49_perc+age_50_69_perc+
             age_70_plus_perc+ post_floyd:conc_dis+t_post_floyd:conc_dis+
             (1|GEOID), 
