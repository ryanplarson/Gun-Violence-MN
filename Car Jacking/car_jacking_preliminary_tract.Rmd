...
#aggregate over years to tract-level
acs_agg <- acs %>%
  select(-year) %>%
  group_by(GEOID) %>%
...
pop_denom_locf <- pop_denom %>% rbind(pop_denom_21)
```
...
cj_spatial <- read_csv("Car Jacking/crime_data.csv")  %>%
  ...
  left_join(pop_denom_locf, by = c("NAME", "year")) %>%
