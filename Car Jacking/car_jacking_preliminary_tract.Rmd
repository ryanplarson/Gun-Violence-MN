...
cj_delta %>% 
  ...
  labs(title = "LISA Plot for Carjacking Change Pre/Post 2020",
       fill = "Cluster Type",
...
hom_delta <- prepost_hom %>%
  mutate(delta = homicide_post-homicide_pre) %>%
  st_sf()
...
hom_delta %>% 
  ...
  labs(title = "LISA Plot for Homicide Change Pre/Post 2020",
       fill = "Cluster Type",
       caption = "Clusters significant at p < .05 with 1,000 simulations.")
```
# MPD Data
...
```{r}
#aggregate MPD data to tract-year
...
re <- lmer(car_jack_rate~as.factor(yearzero)*conc_dis+
             age_below_18_perc+age_19_29_perc+age_30_49_perc+age_50_69_perc+
             age_70_plus_perc+ 
             (1|NAME), 
