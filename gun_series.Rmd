...
ggplot(series)+
  ...
  labs(title = "Figure 1: Weekly Firearm Assaults, 2016-2020",
...
stargazer(ts_ar1,
          title = "Interrupted Time Series Model of Firearm Assaults",
          covariate.labels = c("T","COVID - State of Emergency", "COVID - Stay at Home",
                               "Post-Treatment", "Post-Treatment 3 Months", "AR(1)"),
          dep.var.caption = "Firearm Assaults",
          ...
          notes.label = "Model includes controls for seasonality.")
...
stargazer(fe_full_model,
          title = "Fixed Effects Interrupted Time Series Model of Firearm Assaults",
          covariate.labels = c("T","COVID - State of Emergency", "COVID - Stay at Home",
                               "Post-Treatment", "Post-Treatment 3 Months",
                               "MPD Use of Force t-1", "MPD Stops t-1","MPD Officer Involved Shootings t-1"),
          dep.var.caption = "Firearm Assaults",
          dep.var.labels = "Rate per 1,000",
          model.numbers = FALSE,
          single.row = TRUE,
...
coef <- broom::tidy(fe_int_model$coefficients) %>%
  ...
  mutate(period = ifelse(str_detect(names, "post_floyd_3"), "3+ Months Post-Treatment", "0-3 Months Post-Treatment"),
         main_effect = ifelse(period=="3+ Months Post-Treatment", round(-0.0301406,2), round(-0.5604477,2)),
...
coef_gfs <- gfs
coef_gfs[2,] <- gfs[1,]
coef_gfs$period <- c("3+ Months Post-Treatment", "0-3 Months Post-Treatment")
coef_mpls <- mpls
coef_mpls[2,] <- mpls[1,]
coef_mpls$period <- c("3+ Months Post-Treatment", "0-3 Months Post-Treatment")
...
ggplot() +
  ...
  facet_wrap(~period)+
  labs(title = "Figure 3: Treament Effects by ZCTA", 
       fill = "Coef.")+
  theme(axis.text = element_blank(),
  axis.line = element_blank(),
  axis.ticks = element_blank(),
