...
ggplot(series)+
  ...
  geom_label(aes(x=series$begin_date[series$year==2020 & series$weekofyr==isoweek(date("2020-05-25"))],
                 y=8), 
             label = "George Floyd", show.legend = FALSE)+
  labs(title = "Figure A3: Weekly Firearm Assault+Unintentional Injuries, 2016-2020",
       subtitle = "MHA Hospital Data",
       x = "Week",
       y = "Rate per 100,000 Residents")+
  theme_minimal()+
...
ggplot(series)+
  ...
  geom_label(aes(x=series$begin_date[series$year==2020 & series$weekofyr==isoweek(date("2020-05-25"))],
                 y=.75), 
             label = "George Floyd", show.legend = FALSE)+
  labs(title = "Figure A6: Weekly Firearm Undetermined Injuries, 2016-2020",
       subtitle = "MHA Hospital Data",
       x = "Week",
       y = "Rate per 100,000 Residents")+
  theme_minimal()+
...
$y_t =\beta_0+\beta_1 Time_t + \theta PostKilling_t + \beta_2 TimePost_t+ \mathbf{\phi X}_t+\rho_1 y_{t-1}+\epsilon_t$
...
ts_ar1_pol_unintent <- lm(assault_unintent_incid_c~t+state_of_emerg+stay_at_home+post_floyd+t_post_floyd+
                         tmax_f+snow_in+precip_in+dark_before_12+school+
                  uof_lag+stops_lag+shoot_lag+
                         dplyr::lag(assault_incid_c, 1), data = series)

ts_ar1_pol_undeter <- lm(undeter_incid_c~t+state_of_emerg+stay_at_home+post_floyd+t_post_floyd+
                         tmax_f+snow_in+precip_in+dark_before_12+school+
                  uof_lag+stops_lag+shoot_lag+
                         dplyr::lag(assault_incid_c, 1), data = series)
...
```{r, results='asis', message=FALSE, include = F, warning=FALSE}
stargazer(ts_ar1, ts_ar1_pol, ts_ar1_pol_unintent, ts_ar1_pol_unintent,
...
ggplot() +
  ...
  facet_wrap(~period)+
  scale_fill_distiller(palette = "Spectral")+
  labs(title = "Figure A4: Firearm Assault+Unintentional Injury Rates by ZCTA and Period", 
       subtitle = "MHA Hospital Discharge Data",
       fill = "Incident Rate/100,000")+
  theme(axis.text.x = element_blank(),
        axis.text.y = element_blank(),
...
ggplot() +
  ...
  facet_wrap(~period)+
  scale_fill_distiller(palette = "Spectral")+
  labs(title = "Figure A7: Firearm Undetermined Injury Rates by ZCTA and Period", 
       subtitle = "MHA Hospital Discharge Data",
       fill = "Incident Rate/100,000")+
  theme(axis.text.x = element_blank(),
        axis.text.y = element_blank(),
...
re_base_unintent <- lmer(assault_unintent_incid_c~t+state_of_emerg+stay_at_home+
                 post_floyd+t_post_floyd+
                tmax_f+snow_in+precip_in+dark_before_12+school+
                 uof_lag+stops_lag+shoot_lag+
                 (1|zcta), data = panel)
summary(re_base_unintent)

re_base_unintent_nopol <- lmer(assault_unintent_incid_c~t+state_of_emerg+stay_at_home+
                 post_floyd+t_post_floyd+
                tmax_f+snow_in+precip_in+dark_before_12+school+
                 (1|zcta), data = panel)
summary(re_base_unintent)

re_base_undeter <- lmer(undeter_incid_c~t+state_of_emerg+stay_at_home+
                 post_floyd+t_post_floyd+
                tmax_f+snow_in+precip_in+dark_before_12+school+
                 uof_lag+stops_lag+shoot_lag+
                 (1|zcta), data = panel)
summary(re_base_undeter)
...
#interaction model
re_int_unintent <- lmer(assault_unintent_incid_c~t+state_of_emerg+stay_at_home+
                 post_floyd+t_post_floyd+
                tmax_f+snow_in+precip_in+dark_before_12+school+
                 uof_lag+stops_lag+shoot_lag+
                   med_hh_inc+
...
summary(re_int_unintent)

re_int_undeter <- lmer(undeter_incid_c~t+state_of_emerg+stay_at_home+
                 post_floyd+t_post_floyd+
                tmax_f+snow_in+precip_in+dark_before_12+school+
                 uof_lag+stops_lag+shoot_lag+
                   med_hh_inc+
...
plot_model(re_int_unintent, 
           terms = c("post_floyd", "black_pop", "t[245.5]", "t_post_floyd[15.5]"),
           type = "pred", 
           ci.lvl = 0.95,
           mdrt.values = "meansd",
           title = "Figure A5: Post-Killing X Percent Black Interaction Plot",
           axis.title = c("Post-Killing","Assault+Unintentional Firearm Injuriy Rate per 100,000"))+
  theme_sjplot()+
  ggplot2::labs(colour = "Percent Black")

plot_model(re_int_undeter, 
           terms = c("post_floyd", "black_pop", "t[245.5]", "t_post_floyd[15.5]"),
           type = "pred", 
           ci.lvl = 0.95,
           mdrt.values = "meansd",
           title = "Figure A8: Post-Killing X Percent Black Interaction Plot",
...
stargazer(ts_ar1, ts_ar1_pol, re_base, re_int,
          ...
          dep.var.labels = "Rate per 1,000",
          model.names = FALSE,
          column.labels = c("AR(1) TSR","AR(1) TSRP", "RE HLM","RE HLM +Int."),
          report = "vcs",
          ci=TRUE, 
          ci.level=0.95, 
          ci.separator = "|",
...
```{r, message=F, results='asis'}
class(re_base_unintent) <- "lmerMod"
class(re_int_unintent) <- "lmerMod"

stargazer(ts_ar1_unintent, ts_ar1_pol_unintent, re_base_unintent, re_int_unintent,
          ...
          covariate.labels = c("T","COVID - State of Emergency", "COVID - Stay at Home",
                               ...
                               "Post-Killing X Percent Black"),
          header = F,
          dep.var.caption = "Firearm Unintentional Injuries",
          dep.var.labels = "Rate per 1,000",
          model.names = FALSE,
          column.labels = c("AR(1) TSR","AR(1) TSRP", "RE HLM","RE HLM +Int."),
          model.numbers = TRUE,
...
ggplot() +
  ...
  facet_wrap(~period)+
  scale_fill_distiller(palette = "Spectral")+
  labs(title = "Figure A2: Murder Rates by ZCTA and Period", 
       subtitle = "MPD Data",
