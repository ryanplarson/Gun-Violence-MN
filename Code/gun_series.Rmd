...
re_int <- lmer(assault_incid_c~t+state_of_emerg+stay_at_home+
                ...
                tmax_f+snow_in+precip_in+dark_before_12+school+black_pop+male+
                post_floyd:black_pop:male+post_floyd_3:black_pop+
                 (1|zcta), data = panel)
summary(re_int)
...
stargazer(ts_ar1_pol, fe_full_model,
          ...
          dep.var.caption = "Firearm Assault Injuries",
          dep.var.labels = "Rate per 1,000",
          column.labels = c("Week-Level", "ZCTA-Week-Level"),
          model.numbers = TRUE,
          single.row = TRUE,
          align = T,
          omit = c("zcta", "tmax_f", "snow_in","precip_in","dark_before_12","school"),
          omit.stat = "adj.rsq",
          star.cutoffs = c(.05, .01, .001), star.char = c("*","**","***"),
          add.lines = list(c("ZCTA FE", "No", "Yes")),
          notes.label = "Models include controls for seasonality.")
