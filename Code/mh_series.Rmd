...
ggplot(series)+
  ...
  geom_label(aes(x=series$begin_date[series$year==2020 & series$weekofyr==isoweek(date("2020-05-25"))],
                 y=1.7), 
             label = "George Floyd", show.legend = FALSE)+
  labs(title = "Figure 1: Weekly Black Mental Health Discharges, 2016-2020",
       subtitle = "MHA Hospital Data",
       x = "Week",
...
ggplot(series)+
  ...
  geom_label(aes(x=series$begin_date[series$year==2020 & series$weekofyr==isoweek(date("2020-05-25"))],
                 y=1.25), 
             label = "George Floyd", show.legend = FALSE)+
  labs(title = "Figure 1: Weekly Latino/a Mental Health Discharges, 2016-2020",
       subtitle = "MHA Hospital Data",
       x = "Week",
...
stargazer(ts_ar3, ts_ar3_white, ts_ar3_black, ts_ar3_latin,
          ...
          column.labels = c("OVerall", "White", "Black", "Latio/a"),
          model.numbers = TRUE,
          single.row = TRUE,
          align = T,
          omit.stat = "adj.rsq",
...
ggplot() +
  geom_sf(data = zip_level, aes(geometry = geometry, fill = mh_incid_c)) + 
 ...
 # geom_text_repel(data = gfs, aes(x=lon, y=lat, label = name),
  ...
  labs(title = "Figure X: Mental Health Hospital Discharges by ZCTA and Period", 
...
fe_model <- lm(mh_all_incid_c~t+state_of_emerg+stay_at_home+post_floyd+t_post_floyd+
                          uof_lag+stops_lag+shoot_lag+
                         tmax_f+snow_in+precip_in+
...
#RE base model
re_base <- lmer(mh_all_incid_c~1+
                 (1|zcta)+(1|weekofyr), data = panel)
summary(re_base)

re <- lmer(mh_all_incid_c~t+state_of_emerg+stay_at_home+post_floyd+t_post_floyd+
                          uof_lag+stops_lag+shoot_lag+
                         tmax_f+snow_in+precip_in+
                       (1|zcta)+(1|weekofyr), data = panel)
summary(re)
...
plot_model(re, 
           ...
           ci.lvl = 0.95,
           mdrt.values = "meansd",
           title = "Figure X: Post-Killing X Percent Black Interaction Plot",
