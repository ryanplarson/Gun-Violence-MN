...
ggplot(series)+
  ...
  geom_label(aes(x=series$begin_date[series$year==2020 & series$weekofyr==isoweek(date("2020-05-25"))],
                 y=2), 
             label = "George Floyd", show.legend = FALSE)+
  labs(title = "Figure 2: Weekly Mental Health Discharges by Patient Race, 2016-2020",
       subtitle = "MHA Hospital Data",
       x = "Week",
       y = "Rate per 1,000 Residents",
       fill = "MN COVID-19 Policy",
...
$y_t =\beta_0+\beta_1 Time_t + \theta Event_t + \beta_2 TimePost_t+ \mathbf{\phi X}_t+\rho y_{t-1}+\rho y_{t-2}+\rho y_{t-3}+\epsilon_t$
...
$y_{ti} =\beta_{0i}+\beta_1 Time_t + \theta_i Event_t + \beta_2 TimePost_t+ \mathbf{\phi X}_{ti}+\rho y_{t-1}+\rho y_{t-2}+\rho y_{t-3}+\epsilon_{ti}$
...
re_blk <- lmer(blk_mh_rate~t+state_of_emerg+stay_at_home+post_floyd+t_post_floyd+
                          uof_lag+stops_lag+shoot_lag+
                         tmax_f+snow_in+precip_in+
             conc_dis+
...
re_white <- lmer(white_mh_rate~t+state_of_emerg+stay_at_home+post_floyd+t_post_floyd+
                          uof_lag+stops_lag+shoot_lag+
                         tmax_f+snow_in+precip_in+
             conc_dis+
...
re_latin <- lmer(latin_mh_rate~t+state_of_emerg+stay_at_home+post_floyd+t_post_floyd+
                          uof_lag+stops_lag+shoot_lag+
                         tmax_f+snow_in+precip_in+
             conc_dis+
...
re_coef_map_white <- ggplot() +
  ...
  labs(title = "Figure 3: RE Coefficients-White", 
...
re_coef_map_blk <- ggplot() +
  ...
  geom_text_repel(data = gfs, aes(x=lon, y=lat, label = name),
                ...
                fontface = "bold")+
  scale_fill_distiller(palette = "Spectral")+
  labs(title = "Figure 4: RE Coefficients - Black", 
       subtitle = "Rate per 1,000",
       fill = "Post-Killing Change")+
  theme(axis.text.x = element_blank(),
        axis.text.y = element_blank(),
...
cd_map <- ggplot() +
  ...
  geom_text_repel(data = gfs, aes(x=lon, y=lat, label = name),
                ...
                fontface = "bold")+
  scale_fill_distiller(palette = "Spectral")+
  labs(title = "Figure 5: Concentrated Disadvantage \n by ZCTA", 
...
re_int <- lmer(mh_rate~t+state_of_emerg+stay_at_home+post_floyd+t_post_floyd+
                          uof_lag+stops_lag+shoot_lag+
...
re_int_blk <- lmer(blk_mh_rate~t+state_of_emerg+stay_at_home+post_floyd+t_post_floyd+
                          uof_lag+stops_lag+shoot_lag+
                         tmax_f+snow_in+precip_in+conc_dis+
                 post_floyd:conc_dis+
              dplyr::lag(blk_mh_rate, 1)+ dplyr::lag(blk_mh_rate, 2)+
...
re_int_white <- lmer(white_mh_rate~t+state_of_emerg+stay_at_home+post_floyd+t_post_floyd+
                          uof_lag+stops_lag+shoot_lag+
                         tmax_f+snow_in+precip_in+conc_dis+
                 post_floyd:conc_dis+
              dplyr::lag(white_mh_rate, 1)+ dplyr::lag(white_mh_rate, 2)+
...
re_coef_map_white+re_coef_map_blk+cd_map
```

```{r, results='asis', message=FALSE, include = T}
stargazer(re_white, re_blk,  re_int_white, re_int_blk,
          title = "Interrupted Time Series Random Coefficient Models of Black Mental Health",
          covariate.labels = c("T","COVID - State of Emergency", "COVID - Stay at Home",
                               "Post-Killing", "T Post-Killing",  
                               "MPD Use of Force t-1", "MPD Stops t-1",
                               "MPD Officer Involved Shootings t-1",
                               ...
                               "AR(1)-White", "AR(2)-White", "AR(3)-White",
                               "AR(1)-Black", "AR(2)-Black", "AR(3)-Black",
                               "Post-Floyd X Conc.Disad."),
          dep.var.caption = "Mental Health Discharges",
          dep.var.labels.include = FALSE,
          column.labels = c("White", "Black", 
                            "White w/ Int.","Black w/ Int."),
          model.numbers = TRUE,
          single.row = FALSE,
          align = T,
          omit.stat = "adj.rsq",
